...
func (this *Stream) initStream(){
	...
	this.buildPayload = func (subStream Stream, payloadType byte, length int, isEvent bool, payloadChannel chan interface{}, payloadErrorChannel chan error){
		//fmt.Println("stream.buildPayload")
...
func (this *Stream) pushBack(b byte){
	//fmt.Println("len=", len(this.back), "cap=", cap(this.back), "pos=", this.backPos, "back=", this.back)
...
func (this *Stream) readNBytes(n int64) ([]byte, int, error){
	...
	//fmt.Println("n=", n, "this.backPos=", this.backPos, "back=", this.back, "channelBytes=", channelBytes)
	//fmt.Println("backBytes=", backBytes )
	...
	if channelBytes >= 0 || n == ReadAllPayload {
		...
		//fmt.Println("R=", r.n, "OK=", ok)
		if ok && (channelBytes >= 0 && channelBytes != r.n) {
			fmt.Println("Buffer read, but size not exptected channelBytes=", channelBytes, r.n)
...
func readColumnValueTypeBytes(schema, table, column string, metaDef []byte, stream *Stream) (interface{}, error){
	...
	if n, err = stream.ReadUintLenenc(); err == nil{
		fmt.Println("Length of readColumnValueTypeBytes=", n)
		buf, _, err = stream.readNBytes(int64(n))
	}
	return interface{}(buf), err
}
func readColumnValueTypeString(schema, table, column string, metaDef []byte, stream *Stream) (interface{}, error) {
	...
	if ret != nil{
		if buf, ok := ret.([]byte); ok{
			...
			panic("ColumnTypeString returned no []byte")
		}
	}
	return s, err
}
...
func readColumnValueTypeInt16(schema, table, column string, metaDef []byte, stream *Stream) (interface{}, error){
	...
	var buf []byte
	var ret int16
	fmt.Println("IN readColumnValueTypeInt16")
...
func readColumnValueTypeDatetime(schema, table, column string, metaDef []byte, stream *Stream) (interface{}, error){
	...
	if l, err = stream.ReadUint1(); err == nil{
		fmt.Println("readColumnValueTypeDatetime length=", l)
		switch(l){
			case 0:
			format = 0
			case 4, 7, 11:
...
func init(){
	...
	readColumnValueFunc[ColumnTypeNull       ] = func(schema, table, column string, metaDef []byte, stream *Stream) (interface{}, error) { return nil,nil }
	readColumnValueFunc[ColumnTypeTimestamp  ] = func(schema, table, column string, metaDef []byte, stream *Stream) (interface{}, error) {
		...
		if int32Val, ok := val.(int32); ok{
			...
			panic("ColumnTypeTimestamp not int32")
		}
		return ret, err
	}
	...
	readColumnValueFunc[ColumnTypeNewDate    ] = func(schema, table, column string, metaDef []byte, stream *Stream) (interface{}, error) { return nil,nil }
	...
	readColumnValueFunc[ColumnTypeMediumBlod ] = readColumnValueTypeBytes
	...
	readColumnValueFunc[ColumnTypeBlob       ] = func(schema, table, column string, metaDef []byte, stream *Stream) (interface{}, error) {
		...
		fmt.Println("maxBytes in ColumnTypeBlob=", maxBytes)
	...
	readColumnValueFunc[ColumnTypeString     ] = func(schema, table, column string, metaDef []byte, stream *Stream) (interface{}, error) {
		...
		if metaDef[0] == byte(ColumnTypeSet) || metaDef[0] == byte(ColumnTypeEnum) {
			...
			fmt.Println("buf=", buf)
			if err == nil{
				if tableColumnSetTypeValues, ok := stream.serverConfig.ColumnSetTypeValues[schema]; ok{
					if columnSetTypeValues, ok := tableColumnSetTypeValues[table]; ok{
						if values, ok := columnSetTypeValues[column]; ok{
							n := countMask(buf, len(values))
							fmt.Println("len=", len(values), " n=", n)
							...
							for i := range buf{
								for j := 0; j < 8; j++{
									...
									m := 1 << j
									if buf[i] & byte(m) != 0{
										fmt.Println("SET i=", i, " j=", j, " index=", index, " value=", values[index])
...
func(this *Stream) readColumnValue(schema, table, column string, columnType ColumnType, metaDef []byte, isNul bool) (ColumnValueType, error){
	...
	var err error
	fmt.Println("IN readColumnValue columnType=", columnType, " metaDef=", metaDef, " isNul=", isNul)
	if !isNul{
		if f, ok := readColumnValueFunc[columnType]; ok{
			fmt.Println("readColumnValue columnType=", columnType)
...
func init(){
								...
								for end := stream.byteReadCounter + int64(ret.StatusVarsLength); stream.byteReadCounter < end; {
									if key, err := stream.ReadUint1(); err == nil{
										...
										switch(key){
											...
											panic("Q_LC_TIME_NAMES_CODE")
											statusVar.uint2Val1, err = stream.ReadUint2()

											case Q_CHARSET_DATABASE_CODE:
											panic("Q_CHARSET_DATABASE_CODE")
											statusVar.uint2Val1, err = stream.ReadUint2()

											case Q_TABLE_MAP_FOR_UPDATE_CODE:
											panic("Q_TABLE_MAP_FOR_UPDATE_CODE")
											statusVar.uint8Val, err = stream.ReadUint8()

											case Q_MASTER_DATA_WRITTEN_CODE:
											panic("Q_MASTER_DATA_WRITTEN_CODE")
											statusVar.uint4Val, err = stream.ReadUint4()
	...
	// SLAVE_EVENT 忽略
	createEventFuncs[EventTypeSlaveEvent] = func(int, EventHeaderType, *Stream) (interface{}, error){ panic("SLAVE_EVENT"); return nil, nil }
	// CREATE_FILE_EVENT
	createEventFuncs[EventTypeCreateFileEvent] = func(packetLength int, _ EventHeaderType, stream *Stream) (interface{}, error){
		panic("CREATE_FILE_EVENT")
	...
	createEventFuncs[EventTypeExecuteLoadQueryEvent] = func(_ int, _ EventHeaderType, stream *Stream) (interface{}, error){
		panic("EXECUTE_LOAD_QUERY_EVENT")
	...
	createEventFuncs[EventTypeTableMapEvent] = func(_ int, _ EventHeaderType, stream *Stream) (interface{}, error){
		...
		var err error
		fmt.Println("stream.serverConfig.EventTypeHeaderLength[TABLE_MAP_EVENT]=", stream.serverConfig.EventTypeHeaderLength[EventTypeTableMapEvent-1] )
	...
	readRowColumnValues := func(schema, table, column string, nulBitMap []byte, tableMapEvent TableMapEventType, stream *Stream)([]ColumnValueType, error){
		...
		metaDefPosition := 0
		fmt.Println("IN readRowColumnValues, columnTypes=", columnTypes)
		for i := range columnTypes{
			...
			if metaDefLength, ok := columnMetaDefLength[columnTypes[i]]; ok{
				...
				// TODO 这个长度没指定
				panic(fmt.Sprintf("Donot konw metaDefLength for %v", columnTypes[i]))
				break
			}
		}
		return ret, err
	...
	createRowEvent := func(eventType Uint1, rowEventVersion Uint1, payloadLength int, stream *Stream) (interface{}, error){
		...
		var err error
		fmt.Println("stream.serverConfig.EventTypeHeaderLength[eventType - 1]=", stream.serverConfig.EventTypeHeaderLength[eventType - 1])
				...
				if err == nil{
					...
					if err == nil{
						...
						if tableMap, ok := stream.serverConfig.TableMaps[Uint8(ret.TableId)]; !ok{
							// 这个表未定义？
							fmt.Println("TableMap undefined, TableId=", ret.TableId)
						...
						for{
							...
							// TODO:这里要改的地方很多
							fmt.Println("aaa Schema=", schema, " Table=", table)
							column = ""
							...
							fmt.Println("Schema=", schema, " Table=", table, " Column=", column)
							...
							if rowsEventRow.NulBitmap1, _, err = stream.readNBytes(int64((bitCount + 7 )/ 8)); err == nil{
								...
								fmt.Println("is UpdateV1 or V2?=", (eventType == EventTypeUpdateRowsEventv1 || eventType == EventTypeUpdateRowsEventv2), " err=", err, " byteReadCounter=", stream.byteReadCounter )
								if err == nil && (eventType == EventTypeUpdateRowsEventv1 || eventType == EventTypeUpdateRowsEventv2) {
									bitCount = countMask(ret.ColumnsPresentBitmap2, int(ret.NumberOfColumns))
									fmt.Println("update bitCount=", bitCount)
									if rowsEventRow.NulBitmap2, _, err = stream.readNBytes(int64((bitCount + 7 )/ 8)); err == nil{
										// 读入各字段的value
										fmt.Println("NulBitmap2=", rowsEventRow.NulBitmap2)
										fmt.Println("Valu2 tableMapEvent=", tableMapEvent)
										rowsEventRow.Value2, err  = readRowColumnValues(schema, table, column, rowsEventRow.NulBitmap2, tableMapEvent, stream)
										fmt.Println("Value2 End, byteReadCounter=", stream.byteReadCounter)
									}else{
										// TODO 这个表的结构未知
									}
								}
	...
	// WRITE_ROWS_EVENTv0
	createEventFuncs[EventTypeWriteRowsEventv0] = func(payloadLength int, _ EventHeaderType, stream *Stream) (interface{}, error){ panic("WRITE_ROWS_EVENTv0"); return createRowEvent(EventTypeWriteRowsEventv0, 0, payloadLength, stream) }
	// UPDATE_ROWS_EVENTv0
	createEventFuncs[EventTypeUpdateRowsEventv0] = func(payloadLength int, _ EventHeaderType, stream *Stream) (interface{}, error){ panic("UPDATE_ROWS_EVENTv0"); return createRowEvent(EventTypeUpdateRowsEventv0, 0, payloadLength, stream) }
	// DELETE_ROWS_EVENTv0
	createEventFuncs[EventTypeDeleteRowsEventv0] = func(payloadLength int, _ EventHeaderType, stream *Stream) (interface{}, error){ panic("DELETE_ROWS_EVENTv0"); return createRowEvent(EventTypeDeleteRowsEventv0, 0, payloadLength, stream) }
	// WRITE_ROWS_EVENTv1
	...
	createEventFuncs[EventTypeHeartBeatEvent] = func(payloadLength int, _ EventHeaderType, stream *Stream) (interface{}, error){ panic("HEARTBEAT_EVENT"); return nil, nil }
	// 忽略
	createEventFuncs[EventTypeIgnorableEvent] = func(payloadLength int, _ EventHeaderType, stream *Stream) (interface{}, error){ panic("IGNORABLE_EVENT"); return nil, nil }
	// ROW_QUERY_EVENT
	createEventFuncs[EventTypeRowsQueryEvent] = func(payloadLength int, _ EventHeaderType, stream *Stream) (interface{}, error){
		panic("ROW_QUERY_EVENT")
...
func (this *Stream) readEventPacket(packetLength int)(ret interface{}, err error){
	this.reset()
...
func (this *Stream) readImpl(isEvent bool)(ret interface{}, err error){
	...
	if this.config != nil{
		dumpStream("S->C", packet.PayloadLength, this.config.buf)
	}
	err = e
	return ret, err
}
...
func dumpStream(dir string, PayloadLength Uint3, buf []byte){
	if buf != nil{
